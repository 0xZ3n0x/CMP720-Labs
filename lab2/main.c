...
void uart_init(void) {
    ...
    const uint32_t fclk = 84000000;
    const uint32_t baudrate = 115200;
    const uint32_t oversampling_divisor = 16;

    USART1_BRR = fclk / (baudrate * oversampling_divisor);
...
void led_init(void) {
    ...
    const uint32_t low_bit  =  2 * LED_GREEN_PIN;
    const uint32_t high_bit =  2 * LED_GREEN_PIN + 1;

    GPIOD_MODER &= ~(1 << high_bit | 1 << low_bit);
    GPIOD_MODER |= 1 << low_bit;
